...
(function () {
    'use strict';
    const regex = /^(\d\d):(\d\d)\s+(\d\d):(\d\d)\s+(\d?\d?.?\d?\d)?\s?(.+)$/;
    const commentRegex = /^(\D*-\d*)(.*)$/;
    ...
    var jiraBaseUrl = "https://jira.dormakaba.net/secure/Dashboard.jspa#book_";
            ...
            if (enableJiraBooking) {
                ...
                if (commentMatch) {
                    ...
                    if (enableJiraAutoBooking) {
                        jiraUrl = jiraUrl + ";?enableJiraAutoBooking=true";
                    }
                    var newWindow = window.open(jiraUrl);
                }
            }
        }
    }
